...
export default function Model(props) {
  ...
  const wheelMaterial = new THREE.MeshStandardMaterial().clone(
    materials.plastic
  );
  const windMaterial = new THREE.MeshStandardMaterial().clone(
    materials.plastic
  );
  const frontAndBackMaterial = new THREE.MeshStandardMaterial().clone(
    materials.plastic
  );
  const silverWheelMaterial = new THREE.MeshPhysicalMaterial().clone(
    materials.silver
  );
  const silverFrontAndBackMaterial = new THREE.MeshPhysicalMaterial().clone(
    materials.silver
  );
  useFrame((state, delta) => {
    easing.dampC(
      materials["Material.001"].color,
      data.insideWheelColor,
      0.25,
      delta
    );
    easing.dampC(materials.paint.color, data.bodyColor, 0.25, delta);
    easing.dampC(materials.plastic.color, data.bodyColor, 0.25, delta);
    easing.dampC(
      frontAndBackMaterial.color,
      data.frontAndBackColor,
      0.25,
      delta
    );
    easing.dampC(silverFrontAndBackMaterial.color, "#fff3ee", 0.25, delta);
    easing.dampC(windMaterial.color, data.windColor, 0.25, delta);
    easing.dampC(wheelMaterial.color, data.wheelColor, 0.25, delta);
    easing.dampC(silverWheelMaterial.color, data.wheelColor, 0.25, delta);
    easing.dampC(materials.silver.color, "#ffffff", 0.25, delta);
  });
  console.log(wheelMaterial);
  console.log(materials.plastic);
  return (
    <group {...props} dispose={null}>
      ...
      >
        ...
        <mesh
          ...
          material={materials.plastic} //Primary Light
        />
        <mesh
        ...
        <mesh
          ...
          material={silverFrontAndBackMaterial}
        />
        <mesh
          geometry={nodes.boot010_0.geometry}
          material={frontAndBackMaterial} //Front and back
        />
